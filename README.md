...
设备业务链路默认使用 Type-C 口上的 USB-Serial/JTAG CDC，Wi-Fi WebSocket 自动兜底；
帧格式、切换条件和烧录注意事项见 [`docs/usb-transport.md`](docs/usb-transport.md)。
...
Bridge 内部 Provider 通过 `providers.Update{Notification: ...}` 投递业务对象；同进程的
其他组件也可调用 `bridge.PublishNotification(...)`。进程外调用使用已随 `serve` 启动的
`POST /v2/notifications`，请求携带 Bridge Token 和完整 v2 notification envelope。
两种入口共享校验、TTL、去重、revision、历史与设备广播链路，不需要另起服务。
可直接运行的 `curl` 示例、返回码和所有权约束见
...
升级 Codex 上游版本后，可用
[`patches/codex-token-rate-rust-v0.144.4`](patches/codex-token-rate-rust-v0.144.4/README.md)
中的版本化 patch series 一键重新应用插桩、运行针对性测试、release 编译并安装；不要在
更新上游时手工复制零散源码。

看到新的稳定 release tag 后，直接运行：

```bash
./scripts/update-patched-codex.sh                 # 自动选择最新 rust-vX.Y.Z
./scripts/update-patched-codex.sh rust-v0.145.0  # 或显式指定 tag
```

脚本会从 `origin` 拉取 tags，在干净的 `../codex` 中创建
`patch/token-rate-<tag>` 分支，然后应用 patch、测试、release 编译、安装并刷新
AgentBeacon 服务。出现 `git am` 冲突时会保留冲突现场并立即退出，不会自动跳过或覆盖。
